...
if (isset($_SESSION["user_id"])) {
    ...
    $event_data = null;
    $totalQuery = null;
    $no_event_header = null;
    $no_event_message = null;
    $no_event_button = null;

    if (isset($_POST["activePage"]) && $_POST["activePage"] === "ManageEvents") {
        // For Search Filter
        $search = $_POST['search'] ?? '';
        $searchLike = "%" . $search . "%";

        // Fecting Data from event table
        $event_data = $connection->prepare("SELECT * FROM faculty_coordinators
                                                            INNER JOIN event ON event.event_id = faculty_coordinators.event_id 
                                                            WHERE user_id = ?
        ...
        /* TOTAL COUNT */
        $totalQuery = $connection->prepare("SELECT COUNT(*) as total FROM faculty_coordinators 
                                                                            INNER JOIN event ON event.event_id = faculty_coordinators.event_id 
                                                                            WHERE user_id = ?
                                                                            AND(? = '' OR event_name LIKE ? OR DATE_FORMAT(event_start_date, '%d %M %Y') LIKE ? OR event_venue LIKE ? OR registration_fee LIKE ? OR TIME_FORMAT(event_start_time, '%h:%i %p') LIKE ? OR TIME_FORMAT(event_end_time, '%h:%i %p') LIKE ? OR organized_by LIKE ?)");

        $no_event_message = "You are not assigned as a coordinator for any events yet. Once assigned, you can manage event details here.";
        $no_event_header = "No Assigned Events Yet 🎟️";
        $no_event_button = "Create Event";
    } else if (isset($_POST["activePage"]) && $_POST["activePage"] === "MyEvents") {
        // For Search Filter
        $search = $_POST['search'] ?? '';
        $searchLike = "%" . $search . "%";
        ...
        $no_event_message = "You haven’t created any events yet. Start by organizing an event and make it available for students today.";
        $no_event_header = "No Events Created Yet 🎟️";
        $no_event_button = "Create Event";
    } else if (isset($_POST["activePage"]) && $_POST["activePage"] === "UpcomingEvents") {
        // For Search Filter
        $search = $_POST['search'] ?? '';
        $searchLike = "%" . $search . "%";

        $event_data = $connection->prepare("SELECT event.* FROM event 
                                                                INNER JOIN faculty_coordinators ON faculty_coordinators.event_id = event.event_id 
                                                                WHERE faculty_coordinators.user_id = ?
                                                                AND NOW() < TIMESTAMP(event_start_date, event_start_time) AND cancelled = 'No'
                                                                AND(? = '' OR event_name LIKE ? OR DATE_FORMAT(event_start_date, '%d %M %Y') LIKE ? OR event_venue LIKE ? OR registration_fee LIKE ? OR TIME_FORMAT(event_start_time, '%h:%i %p') LIKE ? OR TIME_FORMAT(event_end_time, '%h:%i %p') LIKE ? OR organized_by LIKE ?)
        ...
        $totalQuery = $connection->prepare("SELECT COUNT(*) as total FROM event
                                                                            INNER JOIN faculty_coordinators ON faculty_coordinators.event_id = event.event_id
                                                                            WHERE faculty_coordinators.user_id = ?
                                                                            AND NOW() < TIMESTAMP(event_start_date, event_start_time)
                                                                            AND(? = '' OR event_name LIKE ? OR DATE_FORMAT(event_start_date, '%d %M %Y') LIKE ? OR event_venue LIKE ? OR registration_fee LIKE ? OR TIME_FORMAT(event_start_time, '%h:%i %p') LIKE ? OR TIME_FORMAT(event_end_time, '%h:%i %p') LIKE ? OR organized_by LIKE ?)");

        $no_event_message = "There are no upcoming events right now. Create a new event for students to register.";
        $no_event_header = "No Upcoming Events Available 🎟️";
        $no_event_button = "Create Event";
    }

    $event_data->bind_param("sssssssssii", $_SESSION["user_id"], $search, $searchLike, $searchLike, $searchLike, $searchLike, $searchLike, $searchLike, $searchLike, $limit, $offset);
    $event_data->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        $totalQuery->bind_param("sssssssss", $_SESSION["user_id"], $search, $searchLike, $searchLike, $searchLike, $searchLike, $searchLike, $searchLike, $searchLike);
        $totalQuery->execute();
        ...
        while ($data = $result->fetch_assoc()) {
            ...
            if ($data["cancelled"] === "Yes") {
                ...
                if ($_POST["activePage"] === "ManageEvents") {
                    if (strtotime($currentDateTime) < strtotime(($eventDateTimeStart))) {
                        $edit = "<a href='CancelEventForm.php?event_id={$event_id}' class='btn-book mt-3' id='btn-red'>
                                <i class='fa-solid fa-xmark'></i> Cancel Event
                            </a>";

                        $view_details = "<a href='AddEvent.php?event_id={$event_id}' class='btn-book mt-3'>
                                        <i class='fa-solid fa-pen-to-square'></i> Edit Event
                                    </a>";

                        $temp = $view_details . $edit;
                    } else {
                                            ...
                                            <div class='col-lg-8'>
                                                ...
                                                <div class='d-flex gap-2 flex-wrap'>
                                                    <a
                                                        ...
                                                        <i class='bi bi-calendar-plus'></i> {$no_event_button}
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
